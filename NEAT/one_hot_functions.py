...
def encode_cards(player, current_trick, tricks_per_player):
    ...
    cards_in_current_trick = [card for _, card in current_trick]
    encoding = []
    ...
    for card in all_cards:
        ...
        elif card in cards_in_current_trick:
            encoding.extend([0, 1, 0, 0, 0, 0, 0])
        else:
            # Check if the card is in any player's won tricks
            found = False
            player_idx = 0
            for _, tricks in tricks_per_player.items():
                player_idx += 1
                for trick in tricks:
                    if card in trick:
                        encoding.extend([0] * (player_idx + 1) + [1] + [0] * (5 - player_idx))
                        found = True
                        break
                if found:
                    break
...
def encode_players(players, start_player, round_scores, game_scores, kontra_player, re_player, tout_player, klopfen_players):
    ...
    def player_encoding(player):
        ...
        absolute_position[players.index(player)] = 1
